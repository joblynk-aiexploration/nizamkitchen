...
const DROPBOX_ROLES: PlatformRole[] = ["platform_owner", "platform_admin", "support_admin", "country_manager"];
...
export function assertDropboxAccess(session: StorageSession) {
  assertPlatformRole(session.user.platformRole, DROPBOX_ROLES);
}
...
export function canAccessAdminDropboxFile(session: StorageSession, file: Pick<StorageFile, "organizationId" | "uploadedById" | "visibility" | "status" | "countryCode">) {
  if (file.status === "deleted" || file.status === "quarantined") return false;
  if (session.user.platformRole === "platform_owner" || session.user.platformRole === "platform_admin" || session.user.platformRole === "support_admin") return true;
  if (session.user.platformRole === "country_manager") {
    const assigned = session.countryAssignments?.map((assignment) => assignment.countryCode) ?? [];
