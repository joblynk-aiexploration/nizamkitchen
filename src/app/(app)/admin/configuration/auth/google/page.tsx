...
export default async function GoogleAuthConfigurationPage({
  searchParams,
}: {
  ...
  const [params, integrations, publicConfig] = await Promise.all([
    ...
    getPublicIntegrationConfig(IntegrationProvider.google_oauth),
  ]);
  const integration = integrations.find((item) => item.provider === "google_oauth");
  const callbackUrl = stringSetting(integration?.settings ?? [], "callbackUrl") || new URL("/api/auth/oauth/google/callback", env.APP_URL).toString();
  const returnTo = "/admin/configuration/auth/google";
  ...
  return (
    <AdminShell
      session={session}
      title="Google OAuth"
      description="Manage Google social sign-in, allowed domains, callback URLs, auto-create behavior, and the login button from the platform configuration vault."
    >
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-[var(--color-muted)]">
              Configure this exact callback in your Google Cloud OAuth client:
              <br />
        ...
        <Card>
          ...
          {canManage ? (
            <form action={savePlatformIntegrationAction} className="mt-5 grid gap-4 md:grid-cols-2">
              <input type="hidden" name="provider" value="google_oauth" />
              <input type="hidden" name="category" value="auth" />
              <input type="hidden" name="displayName" value="Google OAuth" />
              <input type="hidden" name="description" value="Google social sign-in configuration" />
              ...
              <div className="md:col-span-2"><Button type="submit">Create Google OAuth config</Button></div>
            </form>
          ) : (
          ...
          <Card>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-sm text-[var(--color-muted)]">
                  Use the vault detail page for advanced country overrides. This page keeps the core Google auth settings in one place.
                </p>
              </div>
              ...
              {canEditSecrets ? (
                <>
                  <form action={savePlatformIntegrationCredentialAction} className="mt-5 grid gap-4">
                    ...
                    <input type="hidden" name="keyName" value="client_id" />
                    <input type="hidden" name="isPublicClientValue" value="true" />
                    <input type="hidden" name="returnTo" value={returnTo} />
                    <TextInput label="Google client ID" name="secretValue" required />
                    <Button type="submit" variant="secondary">Save client ID</Button>
                  </form>
                  <form action={savePlatformIntegrationCredentialAction} className="mt-4 grid gap-4">
                    <input type="hidden" name="integrationId" value={integration.id} />
                    <input type="hidden" name="keyName" value="client_secret" />
                    <input type="hidden" name="returnTo" value={returnTo} />
                    <TextInput label="Google client secret" name="secretValue" type="password" required />
                    <Button type="submit">Save encrypted client secret</Button>
                  </form>
                </>
              ) : (
              ...
              <div className="mt-5 grid gap-4">
                {canManage ? (
                  <>
                    ...
                    <form action={savePlatformIntegrationSettingAction} className="grid gap-3">
                      <input type="hidden" name="integrationId" value={integration.id} />
                      <input type="hidden" name="settingKey" value="allowedDomains" />
                      <input type="hidden" name="returnTo" value={returnTo} />
                      <TextInput label="Allowed email domains" name="settingValueText" defaultValue={stringSetting(integration.settings, "allowedDomains")} placeholder="example.com, partner.org" />
                      <Button type="submit" variant="secondary">Save allowed domains</Button>
                    </form>
                  ...
                  },
                  {
                    ...
                    label: "Show Google button on login/register",
