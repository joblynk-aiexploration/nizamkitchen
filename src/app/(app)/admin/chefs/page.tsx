...
export default async function AdminChefsPage({
  ...
  const [profiles, countries] = await Promise.all([
    listAdminChefProfiles(session, params),
    prisma.country.findMany({ where: { isActive: true }, orderBy: { countryName: "asc" } }),
  ]);

  return (
    <AdminShell session={session} title="Chef marketplace" description="Review, approve, verify, pause, suspend, and manage public chef profiles.">
      ...
      <AdminDataTable
        data={profiles}
        emptyMessage="No chef profiles found."
        columns={[
          {
            key: "profile",
            header: "Profile",
          ...
          { key: "services", header: "Services", render: (profile) => profile.services.length },
        ]}
      />
    </AdminShell>
  );
