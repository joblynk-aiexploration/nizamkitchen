...
import { saveCateringTimeSlotAction } from "../actions";

export const dynamic = "force-dynamic";

export default async function CateringTimeSlotsPage() {
  const session = await requireMembership();
  if (session.activeOrganization.organizationType !== "home_catering") {
    return <EmptyState title="Home catering only" description="Time slots are available for home catering organizations." />;
  }
  const slots = await listSellerTimeSlots(session);
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Home catering" title="Time slots" description="Define pickup, delivery, preorder windows, preparation time, and cutoff rules." />
      <FulfillmentTabs basePath="/catering/fulfillment" />
      <TimeSlotForm action={saveCateringTimeSlotAction} />
