...
import { saveCateringPickupLocationAction } from "../actions";

export const dynamic = "force-dynamic";

export default async function CateringPickupPage() {
  const session = await requireMembership();
  if (session.activeOrganization.organizationType !== "home_catering") {
    return <EmptyState title="Home catering only" description="Pickup settings are available for home catering organizations." />;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Home catering" title="Pickup locations" description="Manage customer pickup addresses, instructions, and default pickup handoff details." />
      <FulfillmentTabs basePath="/catering/fulfillment" />
      <PickupLocationForm action={saveCateringPickupLocationAction} mapsConfig={mapsConfig} />
