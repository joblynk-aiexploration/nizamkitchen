...
import { createRestaurantMenuFromTemplateAction } from "./actions";
...
export default async function RestaurantMenusPage() {
  ...
  const [menus, templates] = await Promise.all([
    ...
    listAvailableMenuTemplates({ usage: "seller", sellerType: "restaurant", countryCode: session.activeOrganization.countryCode }),
  ]);
  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
        ...
        {templates.length === 0 ? (
          <p className="mt-2 text-sm text-[var(--color-muted)]">No active restaurant templates are available yet.</p>
        ) : (
          <form action={createRestaurantMenuFromTemplateAction} className="mt-4 flex flex-wrap gap-3">
